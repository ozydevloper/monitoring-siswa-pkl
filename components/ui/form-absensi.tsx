...
export const FormAbsensi = () => {
  ...
  const handleSubmit = () => {
    ...
    formData.append("status", mutateAbsensi!.status);
    formData.append("image", imageFakePath!);
    formData.append("nama_siswa", mutateAbsensi!.nama_siswa);
    setOnSubmit(true);
    mutateAbsensi!.mutateAsyncFn(formData).then(() => {
      setOnSubmit(false);
    ...
    >
      {mutateAbsensi && (
        <CardParent className="bg-background gap-2 w-full max-w-md h-fit">
          ...
          <div>Form Absensi {mutateAbsensi.status}</div>
